...
export default function ArchivoExpediente({role, expedienteId, documentoId, onChangeState}: ArchivoExpedienteProps) {
    ...
    const fetchFile = async () => {
        ...
        try {
            const response = await fetch(`/api/fileExpediente?expedienteId=${expedienteId}&documentoId=${documentoId}`);

            const data = await response.json();

            if (response.ok) {
                setFileData({nombre: data.nombre, url: data.url, estadoArchivo: data.estadoArchivo});
            } else {
                console.error("Error al obtener la informacion del archivo:", data.error);
            }
        } catch (error) {
    ...
    const handleFileReview = async (approved: boolean) => {
        if (!expedienteId || !documentoId) return
        const newEstado = approved
          ? FILE_STATES.APROBADO
          : FILE_STATES.RECHAZADO
        ...
        const res = await fetch('/api/fileExpediente', {
          ...
          body: JSON.stringify({ expedienteId, documentoId, estadoArchivo: newEstado })
        })
        if (!res.ok) {
          console.error('Error al revisar documento', await res.json())
          // aquí podrías revertir el estado local si falló
        } else {
            const msg = approved ? "El documento ha sido aprobado" : "El documento ha sido rechazado"
            alert(msg)
            onChangeState();
        }
      }
