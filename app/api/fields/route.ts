...
export async function GET(request: NextRequest) {
  try {
    ...
    const fieldsRef = ref(database, `expedientes/expediente${expedienteId}/documentos/${documentoId}/campos`);
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { expedienteId, documentoId, campos, estadoCampos, fieldKey, valor, estado } = body

    if (!expedienteId || !documentoId) {
      return NextResponse.json({ error: 'Faltan expedienteId o documentoId' }, { status: 400 })
    }
    ...
    if (fieldKey) {
      if (valor   !== undefined) updates[`${basePath}/campos/${fieldKey}/valor`]  = valor
      if (estado  !== undefined) updates[`${basePath}/campos/${fieldKey}/estado`] = estado
    }
